...
// Con validaciones, badges de fecha/hora y MODAL DE CONFIRMACION VISUAL antes de guardar
...
async function cargarPronosticosDesdeAPI(jugadorId) {
  ...
  try {
    ...
    if (jugador) {
      // Cargar clasificados por grupo
      GRUPOS_LISTA.forEach(grupo => {
        const clf1 = jugador[`grp_${grupo.toLowerCase()}_clf1`];
        const clf2 = jugador[`grp_${grupo.toLowerCase()}_clf2`];
        if (clf1 && clf1 !== 0) {
          const equipo = equiposCache.find(e => e.id === clf1);
          if (equipo) {
        ...
        if (clf2 && clf2 !== 0) {
          const equipo = equiposCache.find(e => e.id === clf2);
          if (equipo) {
            if (!gruposSeleccion[grupo]) gruposSeleccion[grupo] = {};
      ...
      // Cargar finalistas
      const campeon = jugador.cam ? equiposCache.find(e => e.id === jugador.cam) : null;
      const subcampeon = jugador.sub ? equiposCache.find(e => e.id === jugador.sub) : null;
      const tercero = jugador.ter ? equiposCache.find(e => e.id === jugador.ter) : null;
      const cuarto = jugador.cua ? equiposCache.find(e => e.id === jugador.cua) : null;
      
      if (campeon) finalistasSeleccion.campeon = campeon.name;
      if (subcampeon) finalistasSeleccion.subcampeon = subcampeon.name;
      if (tercero) finalistasSeleccion.tercero = tercero.name;
      if (cuarto) finalistasSeleccion.cuarto = cuarto.name;
...
// ─────────────────────────────────────────────────────────────
// 3. MODAL DE CONFIRMACION VISUAL
// ─────────────────────────────────────────────────────────────
...
function actualizarPuntuacion() {
  ...
  // Actualizar puntos del grupo activo
  const ptsSpan = document.getElementById(`esp-grupo-pts-${grupoActivo}`);
  if (ptsSpan) ptsSpan.textContent = getPtsGrupo(grupoActivo) + ' pts';
}
...
export async function renderizarEspeciales(contenedor, datosCuenta) {
  ...
  // Cargar pronósticos guardados si existen
...
// ─────────────────────────────────────────────────────────────
// 9. EXPONER FUNCIONES GLOBALMENTE PARA EL BOTÓN GUARDAR
// ─────────────────────────────────────────────────────────────
