...
function navegarAEspeciales(ciclo) {
    if (!cambiarVistaCallback) return;
    cambiarVistaCallback('especiales', currentDatosCuenta);
    setTimeout(() => {
        const tabSelector = ciclo === 'ciclo1' ? '.esp-tab[data-tab="ciclo1"]' : '.esp-tab[data-tab="ciclo2"]';
...
function renderizarPreInauguracion(contenedor) {
    ...
    document.querySelectorAll('.ahora-card').forEach(card => {
        ...
        if (accion === 'ciclo1') {
            card.addEventListener('click', () => navegarAEspeciales('ciclo1'));
        } else if (accion === 'ciclo2') {
            card.addEventListener('click', () => navegarAEspeciales('ciclo2'));
        } else if (accion === 'partido-inaugural') {
            card.addEventListener('click', () => abrirModalPartidoInaugural());
        }
    });
}

function renderizarDuranteMundial(contenedor) {
    // Versión simplificada durante el mundial
    contenedor.innerHTML = `
