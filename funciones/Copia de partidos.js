...
// - Tab COLOMBIA muestra solo partidos de la selección Colombia
// - Control por `est` (Velneo)
...
async function cargarPartidos() {
    try {
        ...
        const responseReales = await fetch(`${BASE}/fifa_ptd?api_key=${KEY}&filter[est]=3&_=${timestamp}`);
...
function renderPartidoCard(partido, fechaSim, horaSim, tipoFondo, esPrimerDia = false) {
    ...
    } else if (estadoEst.estado === 'cerrado') {
    ...
    let centroHTML = '';
    if (resultadoReal && estadoEst.estado === 'cerrado') {
        centroHTML = `<div style="font-size:20px; font-weight:700; color:#000;">${resultadoReal.gol_loc} - ${resultadoReal.gol_vis}</div>`;
    } else if (estadoEst.estado === 'pulso50' && marcadorEnVivo) {
        if (marcadorEnVivo.tieneMarcador) {
            centroHTML = `<div style="font-size:18px; font-weight:700; color:#ff3b30;">${marcadorEnVivo.texto}</div>`;
        } else {
    ...
    let pronosticoHTML = '';
    if (pronostico) {
        if (estadoEst.estado === 'cerrado' && resultadoReal) {
...
function refrescarContenido() {
    ...
    if (tabActivo === 'todos') {
        ...
        // Filtrar solo partidos de Colombia
