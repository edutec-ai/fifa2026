...
export function cargarFrontpage(datosCuenta) {
  ...
  // Detectar si es administrador (usuario 'super')
  const esAdmin = datosCuenta.usr === 'super' || datosCuenta.name === 'super' || datosCuenta.nombre === 'super';
  
  // Leer configuración de admin (solo relevante para SUPER)
  ...
  const manejarSeleccionMenu = (opcion, cuenta) => {
    ...
    setTimeout(() => {
      switch(opcion) {
        case 'partidos': if (esAdmin && adminConfig.habilitarPartidos) renderizarPartidos(contenidoContainer, datosCuenta); break;
        case 'especiales': renderizarEspeciales(contenidoContainer, datosCuenta); break;
        case 'tabla': if (esAdmin && adminConfig.habilitarTabla) contenidoContainer.innerHTML = `<div style="text-align:center;color:white;padding:40px;"><h3>📊 Tabla de Posiciones</h3><p>Próximamente...</p></div>`; break;
        case 'la-polla': if (esAdmin && adminConfig.habilitarLaPolla) renderizarPolla(contenidoContainer, datosCuenta); break;
        case 'simulador': renderizarSimulador(contenidoContainer, datosCuenta); break;
        case 'lab': if (esAdmin && adminConfig.habilitarLab) renderizarLab(contenidoContainer, datosCuenta); break;
  ...
  // ============================================================
  // CONSTRUCCIÓN DEL MENÚ SEGÚN EL TIPO DE USUARIO
  // ============================================================
  let opcionesMenu = [];

  if (esAdmin) {
    // USUARIO SUPER: todas las pestañas
    opcionesMenu = [
    ...
    // OTROS USUARIOS: solo Especiales y Simulador
    opcionesMenu = [
      { id: 'especiales', nombre: 'ESPECIALES', color: '#af52de', icono: '⭐' },
      { id: 'simulador', nombre: '💻 SIMULADOR', color: '#ff3b30', icono: '💻' }
    ];
  ...
  // ============================================================
  // TAB BAR MÓVIL SEGÚN EL TIPO DE USUARIO
  // ============================================================
  ...
  if (mobileTabBar) {
    ...
    // Activar primera pestaña por defecto según el tipo de usuario
    let defaultTab = null;
    if (!esAdmin) {
      defaultTab = document.querySelector('.mobile-tab-item[data-opcion="especiales"]');
    } else {
      defaultTab = document.querySelector('.mobile-tab-item[data-opcion="simulador"]');
    }
    if (defaultTab) defaultTab.classList.add('active');
  }
  
  // Cargar contenido por defecto según tipo de usuario
  setTimeout(() => {
    let opcionDefault = 'simulador';
    if (!esAdmin) {
      opcionDefault = 'especiales';
    }
    manejarSeleccionMenu(opcionDefault, datosCuenta);
  }, 100);
  
  // Escuchar cambios en la configuración (solo relevante para SUPER)
