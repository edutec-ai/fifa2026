...
// CONECTADO A API REAL DE RESULTADOS (no sintéticos)
...
let resultadosRealesCache = {}; // NUEVO: para guardar resultados reales de la API
let tabActivo = 'todos';
let grupoActivo = 'A';
...
async function cargarPartidos() {
  try {
    ...
    console.log(`[Partidos] Cargados ${partidosCache.length} partidos, ${Object.keys(resultadosRealesCache).length} con resultados reales`);
...
async function cargarPronosticos(jugId) {
  ...
  try {
    ...
    const data = await response.json();
    const pronosticos = data.fifa_jug_pro || [];
    pronosticosCache = {};
    pronosticos.forEach(p => {
      pronosticosCache[p.ptd] = { s1: p.pro_gol_loc || 0, s2: p.pro_gol_vis || 0 };
    });
    console.log(`[Partidos] Cargados ${Object.keys(pronosticosCache).length} pronósticos del jugador ${jugId}`);
  } catch (error) {
...
function getEstadoPartido(partido, fechaSim, horaSim) {
  ...
  // Priorizar estado real desde la API (est=2 = finalizado)
  ...
  // Si no hay estado real, usar fecha simulada
...
function getResultadoReal(partidoId) {
  ...
  if (real && real.gol_loc !== undefined && real.gol_loc !== null) {
...
function getFasesHabilitadas(fechaSim) {
  ...
  for (const [fase, fechaInicio] of Object.entries(FECHAS_FASES)) {
    if (fechaSim >= fechaInicio) fasesHabilitadas.push(parseInt(fase));
  }
...
function puedePronosticar(partido, fechaSim, horaSim) {
  ...
  if (!fasesHabilitadas.includes(fase)) return false;
  const { estado } = getEstadoPartido(partido, fechaSim, horaSim);
  if (estado === 'finalizado' || estado === 'vivo') return false;
  return true;
}
...
function filtrarPartidosVisibles(partidos, fechaSim) {
  ...
  const fasesHabilitadas = getFasesHabilitadas(fechaSim);
  return partidos.filter(partido => fasesHabilitadas.includes(Number(partido.fas)));
}
...
function renderPartidoCard(partido, fechaSim, horaSim) {
  ...
  // Obtener resultado REAL desde la API (no sintético)
  const resultadoReal = getResultadoReal(partido.id);
  const tieneResultado = resultadoReal !== null;
  const golLocReal = tieneResultado ? resultadoReal.gol_loc : null;
  const golVisReal = tieneResultado ? resultadoReal.gol_vis : null;
...
async function guardarPronostico(ptdId, s1, s2) {
  ...
  try {
    const payload = { jug: currentJugador.id, id: ptdId, pro_gol_loc: s1, pro_gol_vis: s2, pro_res: s1 > s2 ? '1' : s2 > s1 ? '2' : 'X' };
    console.log('📤 Enviando pronóstico:', payload);
    ...
    let respuesta;
    try { respuesta = JSON.parse(responseText); } catch(e) { respuesta = responseText; }
    
    if (response.ok && (respuesta.COD === 1 || respuesta.COD === undefined)) { 
      pronosticosCache[ptdId] = { s1, s2 }; 
      mostrarToast('✅ Pronóstico guardado', 'ok'); 
      refrescarContenido(); 
    } else {
      console.error('Error en respuesta:', respuesta);
      mostrarToast('❌ Error al guardar: ' + (respuesta.DES || 'Código ' + respuesta.COD), 'err');
    }
  } catch (error) { 
