...
// VARIABLE GLOBAL EXPORTADA PARA LA FECHA SIMULADA
...
async function labFetch(api, forceRefresh = false) {
  ...
  if (container) {
    ...
    LAB_APIS.forEach(api => {
      ...
      card.onclick = async (e) => {
        ...
        else { const data = await labFetch(api, true); if (data) labMostrar(api, data); }
      };
