...
export function cargarFrontpage(datosCuenta) {
  ...
  // Construir lista de opciones del menú - TODAS HABILITADAS (INCLUYENDO SIMULADOR)
  const opcionesMenu = [
  ...
  // Construir tab bar para móvil (TODAS LAS OPCIONES, INCLUYENDO SIMULADOR)
  ...
  if (mobileTabBar) {
    ...
    const simuladorTab = document.querySelector('.mobile-tab-item[data-opcion="simulador"]');
    if (simuladorTab) simuladorTab.classList.add('active');
  }
  
  // Cargar simulador por defecto
  setTimeout(() => {
    manejarSeleccionMenu('simulador', datosCuenta);
  }, 100);
